...
| Terminal pace spike | A character typed just after a one-second boundary created a tiny exact final bucket; annualizing `1 char / 24ms` produced a mathematically correct but misleading 500-WPM chart point and distorted consistency. | Accepted the shared raw-data-preserving analysis approach. Canonical buckets remain exact; only a final window below the existing 250ms minimum is combined with its predecessor for the chart and consistency. Rejected visual clipping, padding, deleting the character, or altering overall WPM. Also rederive historical server/guest consistency on read and strengthened guest bucket invariants. | The failing chart regression first showed `avg 89 · peak 500`; after the fix, focused frontend tests passed (19/19), all frontend tests passed (65/65), and PostgreSQL integration passed (17/17) with raw 88.93 WPM, consistency 94.51, and the raw 24ms bucket preserved. |
...
For the terminal spike, the specialists independently agreed on coalescing the
tiny tail but proposed different thresholds. The root selected 250ms instead of
coalescing every partial second because it matches the existing minimum valid
word-test observation and preserves legitimate longer subsecond pace changes.
