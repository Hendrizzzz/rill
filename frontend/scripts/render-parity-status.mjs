...
function validateCatalog(catalog) {
  if (catalog?.schema !== "rill-parity-program/1") {
  ...
  if (
    ...
    !catalog.lanes.some((lane) => lane.id === "rill-quality")
  ) {
...
function validateEvidence(evidence, catalog) {
  if (evidence?.version !== "rill-parity-campaign/1") {
  ...
  const classified =
    ...
    evidence.classifications["rill-rollover-correction"] +
    evidence.classifications["rill-terminal-rollover-correction"] +
  ...
  if (
    evidence.harness?.rillSourceSha256 !== sourceDigest(catalog) ||
...
const lines = [
  ...
  `- Approved TM-023 rollover corrections: **${String(evidence.classifications["rill-rollover-correction"])}**`,
  `- Approved TM-024 terminal-rollover corrections: **${String(evidence.classifications["rill-terminal-rollover-correction"])}**`,
  ...
  `- Rill/harness source SHA-256: \`${evidence.harness.rillSourceSha256}\``,
