...
class ProductionSafetyConfiguration {
    ...
    static BeanFactoryPostProcessor verifiedDatabaseTransport(Environment environment) {
        return beanFactory -> {
            if (environment.getProperty(
                    "typethock.deployment.require-verified-database-tls",
                    Boolean.class,
                    false)) {
    ...
    @Bean
    ApplicationRunner productionSafety(TypeThockProperties properties) {
        return arguments -> validate(properties);
    }

    static void validate(TypeThockProperties properties) {
        ...
        if (!properties.allowedOrigins().isEmpty()) {
            throw new IllegalStateException(
                    "Production is same-origin; typethock.allowed-origins must be empty");
        }
    }
